# 桂林云游：预先渲染的模型画面（2026-09-21）

默认打开的是离线渲好的 WebP，不是手机上现算的 WebGL。逍遥楼用已烘焙的楼阁资产，全城用近景 GLB 换掉远景楼，再在无头 Chrome 里出图。左右滑动换角度，夜景是另一套已经渲好的帧。`?live=1` 或「实时漫游」才加载原来的 Three.js。重新出图：`node scripts/bake-yunyou-plates.mjs`（需要本机 Chrome 和 puppeteer-core）。
