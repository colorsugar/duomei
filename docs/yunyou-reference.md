...
分支 `yunyou-visual-upgrade`。目标：象鼻山—日月双塔—滨江路—解放桥—逍遥楼—东西巷这一段更接近实景，同时整体画面和界面升级。

- 渲染：解析天空（太阳光晕、地平线雾霭）并由同一天空烘出日/夜环境贴图，替换 RoomEnvironment；指数雾与地平线同色；HDR 合成链为 RenderPass（高 DPR 2×、否则 4× MSAA）→ 半分辨率泛光 → OutputPass（色调映射 + 调色/暗角并入同一 pass）。`src/post.js`、`src/atmosphere.js`。
- 水面：Water.js 着色器在 `river-renderer.js` 中打补丁（rf0 0.3→0.035、翡翠色水体、倒影带水色），桌面默认开实时倒影；流畅档水面去掉平铺条纹贴图，波纹随距离衰减。OSM 桥面道路不再画在江面上。
- 远景：`src/karst-horizon.js` 程序化峰林（位置非测绘）与城外田野渐变；地面贴图原先 repeat 1/80 实为纯色，改为 180 m 平铺的中性铺装色。
- 城区补建：`src/city-fill.js` 为 OSM 未记录建筑的街坊补建多层住宅、主干道板楼+裙楼、老城灰瓦坡顶、L 形楼，尺寸/色彩/层高/窗墙比/阳台/底商逐栋随机，东西巷保留区只补坡顶老房并避开巷道与既有铺面。立面着色器读取可选 `facade` 属性，窗格沿墙面切线计算（斜向楼不再拉歪）。补建为风貌示意，不是实测建筑。
- 象鼻山：`scripts/xiangbishan/`（SDF → marching cubes → Blender AO 烘焙 → Draco GLB）。直立临水崖壁、水线溶蚀槽、象鼻石柱与水月洞、圆穹山体、菜花状树冠壳、深色明代砖塔；朝向、洞口、塔与步道坐标沿用原数据。远景由 `city-far.glb` 加载时换入 `xiangbishan-far.glb`。照片引导的近似模型，不是扫描或测绘。
- 解放桥：`scripts/jiefangqiao/build_bridge.py` 按公开数据 5 跨 41.5+61+72+61+41.5 m、宽 45 m 重建空腹式箱型连拱（矢高、腹拱间距、灯柱样式为照片估计），桥轴由旧资产主成分分析得出；夜景拱面蓝光、拱底暖光按站内实拍照片，经 glTF extras `nightStrength` 接入既有夜景。
- 行道树：`scripts/trees/` 重建 `urban-vegetation.glb`（樟树圆冠、榕树伞冠与气根，3 级 LOD，近景撒叶簇卡），接口（species/lod 节点）不变。
- 界面：精华一线快捷条、日/夜快捷按钮、胶囊地标标签（精华一线高亮）、照片满幅背景简介卡、带封面的加载页；云游外壳中音乐唱片缩小并固定在指南针左侧（宽屏仍在左上）。修复手机端“城中导览”换行。
...
- 首屏：loading 不再等待 `city-far.glb`；当前机位精华地标（默认象鼻山）Blender 精模**就绪后再关 loading**（手机优先 `*-far.glb`），其余精华一线后台预载；SDF 仅在 GLB 失败时保留。桌面仍等 `city-far`，且峰林/行道树/城区补建启动时同步。手机峰林、行道树、城区补建仍延到首帧后空闲执行；补建按步长分帧 `yield`；手机补建 step 略疏、峰林数量减半。
...
- 测量（Playwright iPhone 13，CDP 4× CPU + Fast 4G）：首屏可交互约 159s → 67s（−58%）；拖动/自转帧时间 p95 约 300ms → 33ms；桌面对照截图见 `tmp-perf/`。