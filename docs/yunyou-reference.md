...
- 首屏：loading 不再等待 `city-far.glb`；当前机位精华地标（默认象鼻山）Blender 精模**就绪后再关 loading**（手机优先 `*-far.glb`），其余精华一线后台预载；SDF 仅在 GLB 失败时保留。桌面仍等 `city-far` 就绪再关 loading。峰林、行道树点生成、城区补建改到首帧后空闲执行；补建按步长分帧 `yield`；手机补建 step 略疏、峰林数量减半。
