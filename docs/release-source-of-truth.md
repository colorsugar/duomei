...
- “故语” sits between “快活” and “颜色”; its preview holds each book for 1.6 seconds, then uses an event-driven 16-fragment scatter/tint/swap/reassembly transition lasting about 1.1 seconds. A guarded settle phase keeps the incoming fragments visible until the new base cover has decoded and survived two paint frames, preventing the old cover from flashing back. It supports swipe, Arrow/Home/End keys, pause, clickable progress dots, and first/last looping; a normal tap opens `/guyu`.
...
- On the closed cover the top control is `返回故语`; after opening it is replaced by `合上`, which calls the existing preload-safe jump to page zero. Guyu shelf titles wrap completely inside their cards at 320px and wider.
