...
- `/xunji` is the same-origin 寻迹 mount of `xihuan.vercel.app`: `/xunji-src` relays `/`, `/YYYY-MM-DD/`, `/archive/` and optional `/archive/manifest.json`; the React reader is `/xunji`, `/xunji/archive`, `/xunji/YYYY-MM-DD`. Homepage `寻迹` sits immediately before `大陆`; the header and footer items `寻迹` target `/xunji`.
