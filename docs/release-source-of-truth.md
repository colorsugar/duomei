...
- `src/guyu.css`
- `src/main.tsx`
- `public/images/guyu/` public `新说` page assets and their four approved preview covers
- `tokens.css`
- `vercel.json`
...
- Mobile keeps the fixed safe-area shortcut order 首页 / 早报 / 小记 / 故语 / 寻迹 / 云游 / 颜色 / 微言 / Skill; desktop renders the same shortcuts inside the footer. Header still places 寻迹 immediately before 大陆.
- The mobile footer keeps those nine shortcuts on one compact 44px-high row, reduces Guyu shelf-end whitespace, and hides the back-to-top button while the footer intersects the viewport so no link or copyright copy is covered.
...
- `/xunji` is the same-origin 寻迹 mount of `xihuan.vercel.app`: `/xunji-src` relays `/`, `/YYYY-MM-DD/`, `/archive/` and optional `/archive/manifest.json`; the React reader is `/xunji`, `/xunji/archive`, `/xunji/YYYY-MM-DD`. Cards render the full article text (not a two-line teaser), keep a clickable source line, underline body http(s) URLs, and keep a「打开原帖」link from each article's first `a[href^=http]`. Groups use section headings or the known id map (`x-anecdotes` → X琐事, `f-anecdotes` → 影视日常, `s-anecdotes` → 学院校园). Homepage `寻迹` sits immediately before `大陆`; the header and footer items `寻迹` target `/xunji`.
