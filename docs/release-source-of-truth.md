...
- The homepage order remains 主视觉 / 早报 / 小记 / 快活 / 故语 / 云游 / 颜色 / 微言 / 技能 / 版权脚注. 早报 is one card that opens `/zaobao`, a same-tab full-screen `iframe` of `https://zaobao-six.vercel.app` with a visible `← 返回首页` link; the digest content stays external and is never vendored.
...
- Mobile keeps the fixed safe-area shortcut order 首页 / 早报 / 小记 / 故语 / 云游 / 颜色 / 微言 / 技能; desktop renders the same shortcuts inside the footer.
