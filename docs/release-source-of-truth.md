...
- `/zaobao` is a bare-chrome immersive reader: it fetches the daily HTML and archive manifest through the same-origin edge relay `/zaobao-src/*` (never `zaobao-six.vercel.app` from the browser), renders only parsed text/images/source links in the local React layout, and shows a Chinese error panel with 再试一次 / 打开原版 if the relay or parsing fails.
