# EdgeOne 全球不含大陆架构（历史候选说明）

> 当前状态（2026-09-02）：正式站已从仓库根目录部署到现有 EdgeOne Makers 项目 `duomei-guyu`（`makers-brifmhu31vjf`）。本文保留隔离包的数据流和安全设计证据；其中“新建候选、平台值为空、DNS 尚未执行”等操作状态已经过时。当前操作以 `../../../PROJECT_CONTEXT.md` 和 `../../../docs/release-source-of-truth.md` 为准。
...
- 不把原稿、答案、明文 Cookie 或会话密钥写入前端、Git 或静态目录
- 正式部署只提供鉴权后的读取接口，不保留原稿上传、列举或删除接口

## 历史域名门禁（已经完成，禁止重复执行）

以下是首次候选上线前的记录；当时平台下发值为空。当前项目和域名已经投入生产，不得按此列表重新创建或绑定：
