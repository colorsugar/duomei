...
5. 新加坡 Cloud Function 使用平台 Secret 中的最小权限 COS 凭据读取固定私有对象，再以同源 `image/webp` 返回。
...
- 不把原稿、答案、明文 Cookie、会话密钥或 COS 密钥写入前端、Git 或静态目录
...
候选使用全新 Makers 项目、Cloud Functions、COS Bucket 和 CAM 身份。回退仅撤销这次新增的 DNS 记录或解绑候选域名；不修改现有正式站、不删除域名、不迁移或删除原稿。
