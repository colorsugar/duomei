...
- A closed Guyu reader shows `返回故语`; an open reader replaces it with `合上`, which returns to the cover without leaving the route or destroying the reader. Shelf titles must wrap fully inside their card without ellipsis or clipping.
- The homepage Guyu preview displays each book for 1.6 seconds, then keeps 16 visible cover fragments through an approximately 1.1-second scatter/tint/swap/reassembly. After reassembly it keeps the decoded incoming cover layer through a guarded settle phase and two paint frames before revealing the base image, so the prior cover cannot flash back. It supports left/right swipe, Arrow/Home/End keys, a pause control, clickable IG-style progress dots, and seamless first/last looping; a normal tap still opens the public `/guyu` shelf. Reduced-motion mode disables autoplay and uses immediate state changes.
