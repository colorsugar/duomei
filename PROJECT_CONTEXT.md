...
| `/zaobao/archive` | Same-origin archive list from `zaobao-six.vercel.app/archive/manifest.json`; empty state links to the source archive | No |
| `/zaobao/:date` | Same reader for one archived `YYYY-MM-DD` edition fetched from `zaobao-six.vercel.app/<date>/`; invalid dates redirect to `/zaobao/archive` | No |
...
- `/zaobao` removes the global site chrome, extracts only inert text/image/source fields from the CORS-enabled daily HTML, and renders a wide two-column editorial reader on desktop with a direct iframe fallback. Remote scripts are never executed. `/zaobao/archive` and `/zaobao/:date` reuse that reader and the same bare chrome so users browse past editions without leaving `duomei.site`; the homepage `#zaobao` heading and every reader bar expose a `往期` link, and no edition HTML is copied into this repository.
