...
|---|---|---|
| `/` | Full homepage | Yes |
| `/zaobao` | Immersive daily-edition reader; content comes from `zaobao-six.vercel.app` through the same-origin edge relay `/zaobao-src/*`, with a Chinese error panel (再试一次 / 打开原版) as fallback | No |
...
- `/zaobao` removes the global site chrome, extracts only inert text/image/source fields from the daily HTML, and renders a wide two-column editorial reader on desktop. The browser never fetches `zaobao-six.vercel.app` (unreachable from mainland China): the reader, the archive list, and the homepage card all load `/zaobao-src`, `/zaobao-src/YYYY-MM-DD/`, and `/zaobao-src/archive/manifest.json`, served by `edge-functions/zaobao-src*` + `server/zaobaoProxy.mjs`, which fetch the source at the edge and relay only those three document shapes. On failure the reader shows a Chinese error panel with 再试一次 and 打开原版; there is no iframe. Remote scripts are never executed.
