...
const ctx = new Proxy({}, { get: (_t,k) => k==='measureText'?()=>({width:100}):()=>{}, set:()=>true });
...
// Cave center must remain open across its full passage, trunk and vault solid.
for(let x=-178;x<=-116;x+=1)assert(XBS.sdf(x,5,1392)>0,'blocked cave');
assert(XBS.sdf(-151,7,1371)<0,'trunk disconnected');assert(XBS.sdf(-147,23,1391)<0,'vault missing');
...
const ray=new THREE.Raycaster(new THREE.Vector3(-100,5,1392),new THREE.Vector3(-1,0,0));assert.equal(ray.intersectObject(mesh).length,0,'baked cave blocked');
