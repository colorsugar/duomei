...
export function buildSectorBuffers(data) {
  ...
  for(const p of data.buildings){const g=new THREE.ExtrudeGeometry(shape(p),{depth:p.height??12,bevelEnabled:false,steps:1,curveSegments:1});g.rotateX(-Math.PI/2);buckets.building.push(g);}
  ...
  for(const [kind,geos] of Object.entries(buckets)) {
    ...
    result[kind]={position,normal,uv};
  }
  return result;
}
